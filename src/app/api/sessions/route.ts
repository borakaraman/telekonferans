...
export async function POST(req: NextRequest) {
  try {
    ...
    const organizerIdentity = `organizer-host`;
    // Secret returned only to the creator; required to broadcast / manage the floor.
    const hostKey = randomUUID();

    const manager = TranslationSessionManager.getInstance();
    manager.createSession(sessionId, organizerIdentity, hostKey);
